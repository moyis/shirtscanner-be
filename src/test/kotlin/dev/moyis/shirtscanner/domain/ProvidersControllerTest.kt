...
class ProvidersControllerTest : AbstractIntegrationTest() {
    ...
    fun `providers endpoint returns status for providers`() {
        tfs.persistProviderData(
            ProviderData(URI("http://localhost:1234"), ProviderName("ListR1 Test"), ProviderStatus.UP),
            ProviderData(URI("http://localhost:1234"), ProviderName("Yupoo Test"), ProviderStatus.DOWN),
        )
