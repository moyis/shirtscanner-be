...
class ListR1ProductProviderTest{
    ...
    fun `a search on kitsgg returns products`() {
        setUpOkResponseForQuery("france", "kitsgg")
        val result = subject.search("france")
    ...
    fun `a search on 5boundless returns products`() {
        setUpOkResponseForQuery("argentina", "5boundless")
        val result = subject.search("argentina")
        assertThat(result.products).singleElement().satisfies(
    ...
    fun `a search on grkits returns products`() {
        setUpOkResponseForQuery("brazil", "grkits")
        val result = subject.search("brazil")
        assertThat(result.products).singleElement().satisfies(
    ...
    fun `a search on gkkoc returns products`() {
        setUpOkResponseForQuery("argentina", "gkkoc")
        val result = subject.search("argentina")
        assertThat(result.products).singleElement().satisfies(
    ...
    fun `a search on jeofc1 returns products`() {
        setUpOkResponseForQuery("argentina", "jeofc1")
        val result = subject.search("argentina")
        assertThat(result.products).singleElement().satisfies(
            { assertThat(it.name).isEqualTo("2023 Argentina Blue Special Edition Player Version Soccer Jersey") },
            { assertThat(it.productLink.host).isEqualTo(urlBase.host) },
    ...
    fun `a search on jjsport returns products`() {
        setUpOkResponseForQuery("brazil", "jjsport")
        val result = subject.search("brazil")
        assertThat(result.products).singleElement().satisfies(
    ...
    fun `a search on kegaoo returns products`() {
        setUpOkResponseForQuery("argentina", "kegaoo")
        val result = subject.search("argentina")
        assertThat(result.products).singleElement().satisfies(
    ...
    fun `a search on kotofanss returns products`() {
        setUpOkResponseForQuery("argentina", "kotofanss")
        val result = subject.search("argentina")
        assertThat(result.products).singleElement().satisfies(
    ...
    fun `a search on soccer03 returns products`() {
        setUpOkResponseForQuery("argentina", "soccer03")
        val result = subject.search("argentina")
        assertThat(result.products).singleElement().satisfies(
    ...
    fun `a search on fofoshop returns products`() {
        setUpOkResponseForQuery("argentina", "fofoshop")
        val result = subject.search("argentina")
        assertThat(result.products).singleElement().satisfies(
    ...
    fun `a search on aclotzone returns products`() {
        setUpOkResponseForQuery("croatia", "aclotzone")
        val result = subject.search("croatia")
        assertThat(result.products).singleElement().satisfies(
    ...
    fun `a search returns provider url`() {
        setUpOkResponseForQuery("anything")
        val result = subject.search("anything")
        assertThat(result.queryUrl).isEqualTo(URI("${urlBase}${searchQuery("anything")}"))
    }

    @Test
    fun `a search returns empty products when website returns 4xx`() {
        setUp4xxResponseForQuery("anything")
        val result = subject.search("anything")
    ...
    fun `a search returns empty products when website returns 5xx`() {
        setUp5xxResponseForQuery("anything")
        val result = subject.search("anything")
        assertThat(result.products).isEmpty()
    }
    ...
    fun `a search that takes too long should return empty products`() {
        setUpOkResponseForQuery("anything", duration = Duration.ofSeconds(15))
        val result = subject.search("anything")
        assertThat(result.products).isEmpty()
    }
    ...
    private fun setUpOkResponseForQuery(
        ...
        stubFor(
            get(urlPathEqualTo(searchQuery(q)))
                .withHeader("Dnt", equalTo("1"))
                .withHeader("Sec-Ch-Ua", equalTo("\"Not_A Brand\";v=\"8\", \"Chromium\";v=\"120\""))
                .withHeader("Sec-Ch-Ua-Mobile", equalTo("?0"))
                .withHeader("Sec-Ch-Ua-Platform", equalTo("\"macOS\""))
                ...
                .willReturn(ok().withFixedDelay(duration.toMillis().toInt()).withBody(body))
        )
    }
    ...
    private fun setUp5xxResponseForQuery(q: String) {
        stubFor(get(urlPathEqualTo(searchQuery(q)))
            .withHeader("Dnt", equalTo("1"))
            .withHeader("Sec-Ch-Ua", equalTo("\"Not_A Brand\";v=\"8\", \"Chromium\";v=\"120\""))
            .withHeader("Sec-Ch-Ua-Mobile", equalTo("?0"))
            .withHeader("Sec-Ch-Ua-Platform", equalTo("\"macOS\""))
